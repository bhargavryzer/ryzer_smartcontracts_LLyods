...
abstract contract ApproveTransfer is BasicCompliance {
     ...
     */
    function removeApproval(address _from, address _to, uint256 _amount) external onlyAdmin {
     ...
     */
    function approveAndTransfer(address _from, address _to, uint256 _amount) external {
     ...
     */
    function approveTransfer(address _from, address _to, uint256 _amount) public onlyAdmin {
        bytes32 transferId = _calculateTransferID(_from, _to, _amount, address(tokenBound));
     ...
     */
    function _transferProcessed(address _from, address _to, uint256 _amount) internal {
        bytes32 transferId = _calculateTransferID(_from, _to, _amount, address(tokenBound));
        if (_transfersApproved[transferId]) {
            _transfersApproved[transferId] = false;
            emit ApprovalRemoved(_from, _to, _amount, address(tokenBound));
     ...
     */
    function _calculateTransferID(address _from, address _to, uint256 _amount, address _token)
        internal
        pure
        returns (bytes32)
    {
