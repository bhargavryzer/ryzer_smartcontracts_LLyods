...
abstract contract ExchangeMonthlyLimits is BasicCompliance {
     ...
     */
    function complianceCheckOnExchangeMonthlyLimits(address _from, address _to, uint256 _value)
        public
        view
        returns (bool)
    {
        ...
        if (!isTokenAgent(_from) && _from != address(0)) {
            if (isExchangeID(receiverIdentity)) {
                ...
                if (
                    !_isExchangeMonthFinished(receiverIdentity, senderIdentity)
                        && (
                            (
                                getMonthlyCounter(receiverIdentity, senderIdentity) + _value
                                    > _exchangeMonthlyLimit[receiverIdentity]
                            )
                        )
                ) {
                    return false;
